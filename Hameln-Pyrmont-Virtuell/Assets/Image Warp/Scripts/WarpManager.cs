...
namespace Fenderrio.ImageWarp
{
	...
	public class WarpManager
	{
		...
		[MenuItem ("Tools/Image Warp/Fix legacy v1.1 bezier curved edges", false, 203)]
		...
		[MenuItem ("Tools/Image Warp/Fix legacy v1.1 bezier curved edges", true)]
		...
		[SerializeField]
		private Vector3 m_originalCornerPositionBL;
		[SerializeField]
		private Vector3 m_originalCornerPositionTL;
		[SerializeField]
		private Vector3 m_originalCornerPositionTR;
		[SerializeField]
		private Vector3 m_originalCornerPositionBR;
		...
		[SerializeField]
		private Vector3 m_preWarpMeshXDiff;
		[SerializeField]
		private Vector3 m_preWarpMeshYDiff;
		...
		private Vector3 m_cornerHandlePositionBL;
		private Vector3 m_cornerHandlePositionTL;
		private Vector3 m_cornerHandlePositionTR;
		private Vector3 m_cornerHandlePositionBR;
		...
		[SerializeField]
		private bool m_initialisedBezierHandleOffsets = false;
		...
		public Vector3 OriginalCornerPositionBL { get { return m_originalCornerPositionBL; } }
		public Vector3 OriginalCornerPositionTL { get { return m_originalCornerPositionTL; } }
		public Vector3 OriginalCornerPositionTR { get { return m_originalCornerPositionTR; } }
		public Vector3 OriginalCornerPositionBR { get { return m_originalCornerPositionBR; } }

		public Vector3 CornerPositionBL { get { return m_cornerPositionBL; } }
		public Vector3 CornerPositionTL { get { return m_cornerPositionTL; } }
		public Vector3 CornerPositionTR { get { return m_cornerPositionTR; } }
		public Vector3 CornerPositionBR { get { return m_cornerPositionBR; } }

		public Vector3 PreWarpMeshXDiff { get { return m_preWarpMeshXDiff; } }
		public Vector3 PreWarpMeshYDiff { get { return m_preWarpMeshYDiff; } }

		public BezierCurve TopCurve { get { return m_topCurve; } }
		public BezierCurve LeftCurve { get { return m_leftCurve; } }
		public BezierCurve RightCurve { get { return m_rightCurve; } }
		public BezierCurve BottomCurve { get { return m_bottomCurve; } }
		...
		public void PopulateMesh(Vector3[] a_meshVerts, Vector2[] a_meshUvs, WarpManagerInstanceData a_instanceData, ref WarpManagerMeshData a_meshData)
		{
			...
			m_cornerPositionBL = m_originalCornerPositionBL = a_meshVerts [0];
			m_cornerPositionTL = m_originalCornerPositionTL = a_meshVerts [1];
			m_cornerPositionTR = m_originalCornerPositionTR = a_meshVerts [2];
			m_cornerPositionBR = m_originalCornerPositionBR = a_meshVerts [3];

			// Calculate pre warp width/height
			m_preWarpMeshXDiff = m_cornerPositionBR - m_cornerPositionBL;
			m_preWarpMeshYDiff = m_cornerPositionTL - m_cornerPositionBL;
			...
			if (a_instanceData.m_cropLeft != 0 || a_instanceData.m_cropRight != 0)
			{
				...
				m_cornerPositionBL += a_instanceData.m_cropLeft * m_preWarpMeshXDiff;
				m_cornerPositionTL += a_instanceData.m_cropLeft * m_preWarpMeshXDiff;
				m_cornerPositionTR -= a_instanceData.m_cropRight * m_preWarpMeshXDiff;
				m_cornerPositionBR -= a_instanceData.m_cropRight * m_preWarpMeshXDiff;
			}

			if (a_instanceData.m_cropBottom != 0 || a_instanceData.m_cropTop != 0)
			{
				// then vertical
				m_cornerPositionBL += a_instanceData.m_cropBottom * (m_preWarpMeshYDiff);
				m_cornerPositionTL -= a_instanceData.m_cropTop * (m_preWarpMeshYDiff);
				m_cornerPositionTR -= a_instanceData.m_cropTop * (m_preWarpMeshYDiff);
				m_cornerPositionBR += a_instanceData.m_cropBottom * (m_preWarpMeshYDiff);
			}
			...
			if (m_bezierEdges)
			{
				...
				for (int idx = 0; idx < m_vertRowLength; idx++)
				{
					if (idx == 0)
					{
						...
						m_topCurveVertOffsets [0] = Vector3.zero;
						m_bottomCurveVertOffsets [0] = Vector3.zero;
					}
					else if (idx == m_vertRowLength - 1)
					{
						...
						m_topCurveVertOffsets [idx] = Vector3.zero;
						m_bottomCurveVertOffsets [idx] = Vector3.zero;
					}
					else
					{
		...
		{
			float croppedWidth = m_preWarpMeshXDiff.x * (1 - a_warp.cropLeft - a_warp.cropRight);
			float croppedHeight = m_preWarpMeshYDiff.y * (1 - a_warp.cropTop - a_warp.cropBottom);
		...
		public bool OnInspectorGUI (SerializedObject serializedObject)
		{
			...
			SerializedProperty useBezierEdgesProperty = serializedObject.FindProperty ("m_bezierEdges");
			m_guiChanged = GUI.changed;

			EditorGUILayout.PropertyField (useBezierEdgesProperty, new GUIContent("Enabled?"));

			if (useBezierEdgesProperty.boolValue)
			{
				if (!m_guiChanged && GUI.changed && !m_initialisedBezierHandleOffsets)
				{
					// Call to initialise the bezier handle values, so they aren't just Vector3.zero by default
					ResetBezierHandles (serializedObject);

					SerializedProperty warpManagerProperty = serializedObject.FindProperty ("m_warpManager");
					warpManagerProperty.FindPropertyRelative ("m_initialisedBezierHandleOffsets").boolValue = true;
				}
		...
		private void ResetBezierHandles(SerializedObject serialisedObject)
		{
			...
			float croppedWidth = m_preWarpMeshXDiff.x * (1 - leftCrop - rightCrop);
			float croppedHeight = m_preWarpMeshYDiff.y * (1 - topCrop - bottomCrop);
		...
		public bool OnSceneGUI(SerializedObject serializedObject, Transform imageWarpTransform, Vector2 dimensions, Vector2? pivotPoint)
		{
			...
			m_cornerHandlePositionTR = m_imagePosition + (imageWarpTransform.rotation * m_cornerOffset);
			m_cornerHandlePositionBL = m_imagePosition - (imageWarpTransform.rotation * m_cornerOffset);
			m_cornerHandlePositionBR = m_imagePosition + (imageWarpTransform.rotation * (new Vector3(m_cornerOffset.x, -m_cornerOffset.y)));
			m_cornerHandlePositionTL = m_imagePosition + (imageWarpTransform.rotation * (new Vector3(-m_cornerOffset.x, m_cornerOffset.y)));
			...
			CropCorners(ref m_cornerHandlePositionTL, ref m_cornerHandlePositionTR, ref m_cornerHandlePositionBR, ref m_cornerHandlePositionBL, m_leftEdgeCropProperty.floatValue, m_topEdgeCropProperty.floatValue, m_rightEdgeCropProperty.floatValue, m_bottomEdgeCropProperty.floatValue);
			...
			m_newHandlePosition = m_topRightCornerPosition = Handles.PositionHandle (m_cornerHandlePositionTR + (imageWarpTransform.rotation * Vector3.Scale (m_memberProperty.vector3Value, m_currentScale)), imageWarpTransform.rotation);
			if (!m_guiChanged && GUI.changed)
			{
				m_newHandlePosition -= m_cornerHandlePositionTR;
			...
			m_newHandlePosition = m_bottomLeftCornerPosition = Handles.PositionHandle ( m_cornerHandlePositionBL + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale )), imageWarpTransform.rotation);
			if (!m_guiChanged && GUI.changed)
			{
				m_newHandlePosition -= m_cornerHandlePositionBL;
				m_memberProperty.vector3Value = Quaternion.Inverse (imageWarpTransform.rotation) * (new Vector3 (m_newHandlePosition.x / m_currentScale.x, m_newHandlePosition.y / m_currentScale.y, m_newHandlePosition.z / m_currentScale.z));
			}
			...
			m_newHandlePosition = m_bottomRightCornerPosition = Handles.PositionHandle (m_cornerHandlePositionBR + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale )), imageWarpTransform.rotation);
			if (!m_guiChanged && GUI.changed)
			{
				m_newHandlePosition -= m_cornerHandlePositionBR;
				m_memberProperty.vector3Value = Quaternion.Inverse (imageWarpTransform.rotation) * (new Vector3 (m_newHandlePosition.x / m_currentScale.x, m_newHandlePosition.y / m_currentScale.y, m_newHandlePosition.z / m_currentScale.z));
			}
			...
			m_newHandlePosition = m_topLeftCornerPosition = Handles.PositionHandle (m_cornerHandlePositionTL + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale )), imageWarpTransform.rotation);
			if (!m_guiChanged && GUI.changed)
			{
				m_newHandlePosition -= m_cornerHandlePositionTL;
				m_memberProperty.vector3Value = Quaternion.Inverse (imageWarpTransform.rotation) * (new Vector3 (m_newHandlePosition.x / m_currentScale.x, m_newHandlePosition.y / m_currentScale.y, m_newHandlePosition.z / m_currentScale.z));
			}
			...
			if (m_memberProperty.boolValue)
			{
				...
				var fmh_851_64_638121571305409114 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
...
#if UNITY_5_5_OR_NEWER
				var fmh_867_64_638121571305428946 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
...
#if UNITY_5_5_OR_NEWER
				var fmh_884_64_638121571305432485 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
				...
				m_handlePos = m_topLeftCornerPosition + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale ));
#if UNITY_5_5_OR_NEWER
				var fmh_899_64_638121571305435803 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
				Handles.DrawLine (m_topLeftCornerPosition, m_newHandlePosition);
				...
				m_handlePos = m_topRightCornerPosition + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale ));
#if UNITY_5_5_OR_NEWER
				var fmh_916_64_638121571305439123 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
				Handles.DrawLine (m_topRightCornerPosition, m_newHandlePosition);
...
#if UNITY_5_5_OR_NEWER
				var fmh_931_64_638121571305442447 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
				...
				m_handlePos = m_bottomRightCornerPosition + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale ));
#if UNITY_5_5_OR_NEWER
				var fmh_948_64_638121571305445764 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
				Handles.DrawLine (m_bottomRightCornerPosition, m_newHandlePosition);
				...
				m_handlePos = m_bottomLeftCornerPosition + (imageWarpTransform.rotation * Vector3.Scale( m_memberProperty.vector3Value, m_currentScale ));
#if UNITY_5_5_OR_NEWER
				var fmh_963_64_638121571305449062 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
#else
				m_newHandlePosition = Handles.FreeMoveHandle( m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
#endif
				Handles.DrawLine (m_bottomLeftCornerPosition, m_newHandlePosition);
			...
			m_memberProperty = serializedObject.FindProperty ("m_cornerOffsetTR");
			m_cornerHandlePositionTR = m_cornerHandlePositionTR + (imageWarpTransform.rotation * Vector3.Scale (m_memberProperty.vector3Value, m_currentScale));
			m_memberProperty = serializedObject.FindProperty ("m_cornerOffsetBL");
			m_cornerHandlePositionBL = m_cornerHandlePositionBL + (imageWarpTransform.rotation * Vector3.Scale (m_memberProperty.vector3Value, m_currentScale));
			m_memberProperty = serializedObject.FindProperty ("m_cornerOffsetBR");
			m_cornerHandlePositionBR = m_cornerHandlePositionBR + (imageWarpTransform.rotation * Vector3.Scale (m_memberProperty.vector3Value, m_currentScale));
			m_memberProperty = serializedObject.FindProperty ("m_cornerOffsetTL");
			m_cornerHandlePositionTL = m_cornerHandlePositionTL + (imageWarpTransform.rotation * Vector3.Scale (m_memberProperty.vector3Value, m_currentScale));
			...
			if (!serializedObject.FindProperty ("m_bezierEdges").boolValue)
			{
				...
				#if UNITY_5_5_OR_NEWER
				var fmh_1007_64_638121571305453338 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, HandleUtility.GetHandleSize (m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
				#else
				m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
				#endif
				if (!m_guiChanged && GUI.changed)
				{
					...
					m_topEdgeCropProperty.floatValue = CalculateCurrentCropValue (m_newHandlePosition, m_preWarpMeshYDiff.y);
				}
				...
				#if UNITY_5_5_OR_NEWER
				var fmh_1026_64_638121571305456582 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, HandleUtility.GetHandleSize (m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
				#else
				m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
				#endif
				if (!m_guiChanged && GUI.changed)
				{
					...
					m_bottomEdgeCropProperty.floatValue = CalculateCurrentCropValue (m_newHandlePosition, m_preWarpMeshYDiff.y);
				}
				...
				#if UNITY_5_5_OR_NEWER
				var fmh_1045_64_638121571305459799 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, HandleUtility.GetHandleSize (m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
				#else
				m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
				#endif
				if (!m_guiChanged && GUI.changed)
				{
					...
					m_leftEdgeCropProperty.floatValue = CalculateCurrentCropValue (m_newHandlePosition, m_preWarpMeshXDiff.x);
				}
				...
				#if UNITY_5_5_OR_NEWER
				var fmh_1064_64_638121571305462997 = imageWarpTransform.rotation; m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, HandleUtility.GetHandleSize (m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotHandleCap);
				#else
				m_newHandlePosition = Handles.FreeMoveHandle (m_handlePos, imageWarpTransform.rotation, HandleUtility.GetHandleSize(m_handlePos) * BEZIER_HANDLE_SCALE, Vector3.one, Handles.DotCap);
				#endif
				if (!m_guiChanged && GUI.changed)
				{
					...
					m_rightEdgeCropProperty.floatValue = CalculateCurrentCropValue (m_newHandlePosition, m_preWarpMeshXDiff.x);
				}
