Shader "Custom/HoloTriangles" {
	Properties
	{
		...
		_AlphaLimit("AlphaLimit", Range(0.0, 1.0)) = 0.0
		...
		Pass
	{
		...
		{
			float4 col = float4(doMaterial(i.world.xyz, _Center.xyz), 1.0);
			if( col.x <= _AlphaLimit && col.y <= _AlphaLimit && col.z <= _AlphaLimit ){
				col.x = 1;
				col.y = 1;
				col.z = 1;
				col.w = 0;
