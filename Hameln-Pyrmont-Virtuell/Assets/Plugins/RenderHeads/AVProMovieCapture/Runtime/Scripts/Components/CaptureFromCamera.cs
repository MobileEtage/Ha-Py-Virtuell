		...
		private void UpdateTexture()
		{
			...
			if (!HasContributingCameras())
			{
				...
				if (_lastCamera.clearFlags == CameraClearFlags.Nothing || _lastCamera.clearFlags == CameraClearFlags.Depth)
				{
					...
					if (_Transparency == Transparency.Codec)
					{
...
*/
			...
			// Setup rendering a different render target if we're overriding resolution or anti-aliasing
			{
				...
				if (_target == null)
				{
					RenderTextureFormat textureFormat = Utils.GetBestRenderTextureFormat(RequiresHDR(), (_Transparency == Transparency.Codec), IsRealTime);
			...
			if ( !NativePlugin.IsBasicEdition() )
			{
				...
				{
					InitialiseSideBySideTransparency( width, height );
				}

				switch ( _Transparency )
				{
					case Transparency.TopBottom:	outputHeight *= 2;	break;
					case Transparency.LeftRight:	outputWidth *= 2;	break;
				}
			}

			SelectRecordingResolution(outputWidth, outputHeight);
